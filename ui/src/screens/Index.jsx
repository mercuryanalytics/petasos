...
const Index = props => {
    ...
    (user, data) => {
      ...
      switch (content) {
        ...
        case ContentTypes.ManageProject:
          setIsAccessBlocked(
            !(
              hasRoleOnProject(user.id, resId, UserRoles.Viewer) ||
        ...
        case ContentTypes.ManageReport:
          setIsAccessBlocked(
            !(
              hasRoleOnReport(user.id, resId, UserRoles.Viewer) ||
              hasRoleOnClient(user.id, data.client.id, UserRoles.ClientManager)
            )
          )
          break
  ...
  const handleScreenLoad = useCallback(
    user => {
      ...
      switch (content) {
        ...
        case ContentTypes.CreateProject:
          ...
          !client &&
            promises.push(
              dispatch(getClient(+params.clientId)).then(
                action => (client = action.payload),
                error => (dataError = error)
              )
            )
        ...
        case ContentTypes.ManageProject:
          !project &&
            promises.push(
              dispatch(getProject(resId)).then(
                async action => {
        ...
        case ContentTypes.CreateReport:
          ...
          !project &&
            promises.push(
              dispatch(getProject(+params.projectId)).then(
                async action => {
                  project = action.payload
                  return await dispatch(getClient(project.domain_id)).then(
                    action => (client = action.payload),
    ...
    [content, resId, params, checkAuthorizations, dispatch, getCurrentClient, getCurrentProject, getCurrentReport]
  )
  ...
  return !isAccessBlocked && !isDataMissing ? (
    <Screen className={styles.container} private useEmptyState={true} keepLoading={isLoading} onLoad={handleScreenLoad}>
    ...
    (isDataMissing && <PageNotFound />) || (isAccessBlocked && <AccessRestricted />)
  )
}
