...
const Screen = props => {
  ...
  const partnerClient = useSelector(state =>
    !partner ? null : state.clientsReducer.clients.filter(c => c.subdomain === partner)[0] || null
  )
  ...
  const handleSideMenuLoad = useCallback(
    emptyState => {
        ...
        <div className={styles.body}>
          {props.showSideBar !== false && (!realEmptyState || isSuperUser(localUser.id)) && (
            <div className={`${styles.side} ${showSideMenu ? styles.nonHidden : styles.hidden}`}>
              <SideMenu userId={localUser.id} autoselect={!props.independent} onLoad={handleSideMenuLoad} />
            </div>
          )}
