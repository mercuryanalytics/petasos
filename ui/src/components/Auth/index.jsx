...
export const initFromStorage = options => {
  ...
  if (data) {
    if (!data.expiresAt || !data.key || !data.user || +data.expiresAt <= new Date().getTime()) {
...
const Auth = props => {
  ...
  const { config, viewType, redirectTo, logoSrc, onSuccess, passwordResetHandler, passwordChangeHandler, state } = props
  ...
  const isCallback =
    viewType === AuthViewTypes.Login && callbackUrl && callbackUrl.indexOf(history.location.pathname) > -1
  ...
  const handlePasswordResetByToken = useCallback(
    async token => {
      clearErrors()
  ...
  const handlePasswordResetByEmail = useCallback(
    async email => {
      clearErrors()
  ...
  const handleTokenVerification = useCallback(
    async token => {
      ...
      dispatch(verifyPasswordToken(token)).then(
        res => {
  ...
  return !isCallback && (!isConnected || viewType === AuthViewTypes.ChangePassword) ? (
    <div className={styles.container}>
      <div className={styles.object}>
        <div className={styles.header}>
          {!!logoSrc && (
            <div className={styles.logoContainer}>
              {!!logo ? <img className={styles.logo} src={logo} alt="" /> : <Loader inline size={3} />}
            </div>
          )}
