...
const UserMenu = props => {
  ...
  return (
    <HeaderControl className={`${styles.container} ${isOpen ? styles.open : ""}`} onClick={toggleMenu} active={isOpen}>
      <Avatar className={styles.avatar} avatar={avatar} acronym={acronym} />
      <span className={styles.name}>{name}</span>
      {isOpen && (
        <div className={styles.menu}>
          <Link className={styles.menuLink} to={Routes.Account}>
            My account
          </Link>
          <Link className={styles.menuLink} to={Routes.Logout}>
            Log out
          </Link>
        </div>
      )}
