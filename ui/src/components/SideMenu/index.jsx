...
const searchComponentToggles = [{ key: SearchToggles.HideClients, text: "Hide clients", value: false }]
...
const SideMenu = props => {
          ...
          async action => {
            if (action.payload.length) {
              ...
              if (sortedClients.length < 2) {
                await dispatch(getProjects(sortedClients[0].id)).then(
                  action => {
                    if (action.payload.length) {
  ...
  const initProjectCreationRights = useCallback(
    clientId => {
  ...
  const initReportCreationRights = useCallback(
    projectId => {
      let canCreate =
  ...
  const updateSearchResults = useCallback(() => {
    ...
    if (isClientsSearch) {
      ...
      if (Object.keys(matchedClients).length) {
        projects
          .filter(p => !!matchedClients[p.domain_id])
          .forEach(p => {
            matchedProjects[p.id] = true
            pids[p.id] = true
          })
    ...
    if (Object.keys(matchedProjects).length) {
      reports
        .filter(r => !!matchedProjects[r.project_id])
        .forEach(r => {
          rids[r.id] = true
        })
    }
    ...
    projects,
    reports,
    initProjectCreationRights,
    initReportCreationRights,
    clientsSearch,
    projectsSearch,
    activeClient,
    activeProject
  ...
  return (
    <div className={styles.container}>
      ...
      <Scrollable className={styles.menu}>
        {(!isLoading &&
          ((isSearching && !clientsSearch && !isLoadedSearchData) || (!clientsVisibility && !isLoadedProjectsData)) && (
            <Loader inline className={styles.loader} />
          )) ||
          (isSearching && !Object.keys(filteredClients).length && <div className={styles.noResults}></div>) ||
          (!isLoading &&
            !clientsVisibility &&
