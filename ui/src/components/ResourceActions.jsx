...
const ResourceActions = props => {
  ...
  const handleProjectToggle = useCallback(
    async (id, forced) => {
      ...
      if (!loadedProjects[id]) {
        await dispatch(getReports(id)).then(
          async action => {
  ...
  const refreshChildAccessStates = useCallback(
    (type, id, status, clientId) => {
      let newStates = {}
      let projectStates = {},
        clientState = false
  ...
  const setItemStatus = useCallback(
    (type, id, parentId, status, role, scopeId, isGlobal, context) => {
        ...
        if (role) {
          ...
          if (status) {
            ...
            if (role === adminRole) {
              ...
              promises.push(
                dispatch(getAuthorizeAction(options, { role: managerRole, role_state: 1, authorize: true })).then(
                  () => {},
                  () => {}
                )
              )
            ...
            if (role === managerRole) {
              ...
              promises.push(
                dispatch(getAuthorizeAction(options, { role: adminRole, role_state: 0 })).then(
                  () => {},
                  () => {}
                )
              )
          ...
          if (!status) {
            const grantableAccess =
              type === "client" ? UserRoles.ClientAccess : type === "project" ? UserRoles.ProjectAccess : null
            newStates[`${userId}-${type}-${id}-${managerRole}`] = false
            newStates[`${userId}-${type}-${id}-${adminRole}`] = false
            promises.push(
              dispatch(getAuthorizeAction(options, { role: managerRole, role_state: 0 })).then(
                () => {},
                () => {}
              )
            )
            ...
            if (grantableAccess) {
              ...
              promises.push(
                dispatch(getAuthorizeAction(options, { role: grantableAccess, role_state: 0 })).then(
                  () => {},
                  () => {}
                )
              )
      ...
      const hasAnyPermission = !role || role === UserRoles.Viewer ? status : status || getItemStatus(type, id)
  ...
  const setFiltersStatus = useCallback(
    (status, id) => {
      let newState = {}
      if (id) {
        newState[id] = status
  ...
  const getRowActiveClass = useCallback(
    (type, res) => {
      let status
      if (!templateMode) {
    ...
    return (
      <div className={styles.checkboxesTitles}>
        ...
        {!clientId && !!scopes.dynamic && !!getActiveFiltersCount() && (
          <>
            <div className={styles.verticalSeparator} style={{ right: `${getActiveFiltersCount() * 70 + 40}px` }} />
            {scopes.dynamic.map(
              (s, i) =>
                !!filters[s.id] && (
                  <div
  ...
  const hasFilteredProjects = client => {
    let p = projects[client.id].filter(
      project => getRowActiveClass("project", project) !== "" || project.children_access
    )
    return !!p.length
  }
  ...
  const { form, handleSubmit, submitting } = useForm({
    ...
    onSubmit: values => {
      ...
      dispatch(copyPermissions(userId, result)).then(
        action => {
          const handleSuccess = () => {
            form.reset()
            setIsBusy(false)
    ...
    <div className={`${styles.container} ${!clientId ? styles.complete : ""} ${props.className || ""}`}>
      {permissionUpdating && (
        <div className={`${styles.nonClickable}`}>
          <p>Permission updating, please wait.</p>
        </div>
      )}
      {!clientId && (
        <>
          <div className={styles.globals}>
            ...
            >
              ...
              <div className={styles.modalText}>Enter the email address of user you wish to copy its permissions.</div>

              <form className={styles.form} onSubmit={handleSubmit}>
                <Input className={`${styles.modalInput} ${styles.stacked}`} field={copyFromEmail} label="Email" />
                ...
                <span className={styles.spanDescription}>
                  By checking the above checkbox, the users old permission will be retained. If not, they will be
                  overwritten!
                </span>
            ...
            <div className={`${styles.bigTitle} ${styles.justifyBetween}`}>
              ...
              <div className={styles.buttonContainer}>
                <Button onClick={() => setShowOnlyAccess(!showOnlyAccess)}>
                  {showOnlyAccess ? "Show all" : "Show only active permissions"}
                </Button>
                    ...
                    label={renderCheckboxTitle(
                      ...
                      (s.action === "admin" && (
                        <div className={styles.tooltip}>
                          ...
                          <span className={`${styles.item} ${styles.clean}`}>Ability to create/delete Clients</span>
                          <span className={`${styles.item} ${styles.clean}`}>Super Admin interface</span>
                        </div>
                      )) ||
                        (s.action === "research" && (
                          <div className={`${styles.tooltip} ${styles.fixed}`}>
                ...
                <div key={client.id} className={`${styles.client} ${getRowActiveClass("client", client)}`}>
                  ...
                  >
                    ...
                    <Toggle
                      ...
                      onChange={status =>
                        setItemStatus("client", client.id, client.id, status, null, null, null, context)
                      }
                    />
                  ...
                  {!!openClients[client.id] &&
                    (!!loadedClients[client.id] ? (
                      (showOnlyAccess && hasFilteredProjects(client)) ||
                      (!showOnlyAccess && !!projects[client.id] && !!projects[client.id].length) ? (
                        <>
                          {(showOnlyAccess
                            ? projects[client.id].filter(
                                project => getRowActiveClass("project", project) !== "" || project.children_access
                              )
                            : projects[client.id]
                          ).map(project => (
                      ...
                      `}
                            >
                              ...
                              >
                                <div className={styles.info + " first-child"}>
                                  <MdPlayArrow
                                    className={`${styles.arrow} ${!!openProjects[project.id] ? styles.open : ""}`}
                                  />
                              ...
                              {!!openProjects[project.id] &&
                                (!!loadedProjects[project.id] ? (
                                  (showOnlyAccess && hasFilteredReports(project)) ||
                                  (!showOnlyAccess && !!reports[project.id] && !!reports[project.id].length) ? (
                                    (showOnlyAccess
