...
const ReportManage = props => {
  ...
  const data = useSelector(state => (editMode ? state.reportsReducer.reports.filter(r => r.id === id)[0] : null))
  ...
  const init = useCallback(() => {
    ...
    if (id) {
      ...
      let promises = !report
        ? [
            dispatch(getReport(id)).then(
              action => (report = action.payload),
              () => {}
            )
          ]
        : []
  ...
  const { form, handleSubmit, submitting } = useForm({
    ...
    onSubmit: values => {
      ...
      const result = {
        ...
        presented_on: values.presented_on ? format(new Date(values.presented_on), "yyyy-MM-dd") : "",
        project_id: data ? data.project_id : projectId
      }
      if (editMode) {
        data &&
          dispatch(updateReport(data.id, result, result.project_id)).then(
            () => {
              form.reset()
              setIsBusy(false)
              setIsEditClicked(false)
        ...
        dispatch(createReport(result, result.project_id)).then(
          action => {
      ...
      <div className={`${styles.section} ${styles.left}`}>
          ...
          {editMode && (
            <div className={styles.actions}>
              ...
              {editMode && !previewMode && !!data.url && !isEditClicked && (
                <Button link={getAccessibleUrl(data.url, data.name)} target="_blank" action={true}>
                  View report
                </Button>
              )}
            </div>
        ...
        <Scrollable className={styles.scrollableForm}>
          <form className={styles.form} onSubmit={handleSubmit}>
            ...
            />
            {canEdit && isEditClicked && (
              <Input className={styles.formControl} field={url} preview={!canEdit} disabled={isBusy} label="URL" />
            )}
            <Textarea
              className={styles.formControl}
              field={description}
