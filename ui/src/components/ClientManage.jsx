...
const ClientManage = props => {
  ...
  const clients = useSelector(state => (editMode ? state.clientsReducer.clients : null))
  const data = useSelector(state => (editMode ? state.clientsReducer.clients.filter(c => c.id === id)[0] : null))
  ...
  const { form, handleSubmit, submitting, errors } = useForm({
    ...
    onSubmit: values => {
      ...
      if (editMode) {
        ...
        dispatch(createClient(result)).then(
          action => {
            ...
            Promise.all([
              ...
              dispatch(getUsers(client.id, true)).then(
                () => {},
                () => {}
              )
            ]).then(() => {
  ...
  const setTemplateStatus = useCallback(
    status => {
      ...
      dispatch(updateClient(id, { default_template_enabled: status })).then(
        () => {},
        () => {}
      )
    },
  ...
  const changeFieldsTab = useCallback(
    tab => {
      const fields =
        {
  ...
  const renderAvatarUploader = (centered, preview) => {
    const avatarUrl =
      !!avatar && !avatarErrors
        ? avatar.dataURL
        : data && data.logo_url !== Constants.DEFAULT_CLIENT_LOGO_URL
        ? data.logo_url
        : null
    return (
      ...
      <div className={styles.anyActions}>
        <div className={`${styles.title} ${styles.big}`}>
          ...
          {isTablet && (
            ...
            >
              <Close className={`${styles.deleteIcon} ${styles.right}`} />
              Close
            </Button>
          )}
        </div>
          ...
          {((!editMode || tab === ContentTabs.Details) && (
                    ...
                    {(editMode || fieldsTab === FieldsTabs.Details) && (
                      <>
                        <div className={styles.formSection}>
                          ...
                          <div className={styles.controlsGroup}>
                            {editMode && renderAvatarUploader(false, !canEdit)}
                            <div>
                              <Input
                                className={styles.formControl}
                                field={slogan}
                      ...
                      <>
                        ...
                        {!editMode && (
                          <div className={styles.formButtons}>
                            <Button
                              disabled={submitting || isBusy}
                              onClick={() => changeFieldsTab(FieldsTabs.Addresses)}
                            >
                              <span>Continue</span>
                            </Button>
                            {renderCreateCancelButton()}
                          ...
                          <div className={styles.controlsGroup}>
                            ...
                            <div>
                              ...
                              <Input
                                ...
                                value={
                                  billingAsMailing
                                    ? mailing_address_1.input.value
                                    : billingDefaults.billing_address_1 || undefined
                                }
                                persistErrors={persistFieldsErrors}
                                label={`Address Line 1 ${canEdit ? "*" : ""}`}
                              />
                              <Input
                                ...
                                value={
                                  billingAsMailing
                                    ? mailing_address_2.input.value
                                    : billingDefaults.billing_address_2 || undefined
                                }
                                persistErrors={persistFieldsErrors}
                                label="Address Line 2"
                              />
                              <Input
                                ...
                                value={
                                  billingAsMailing
                                    ? mailing_city.input.value
                                    : billingDefaults.billing_city || undefined
                                }
                                persistErrors={persistFieldsErrors}
                                label={`City ${canEdit ? "*" : ""}`}
                              />
                              <Input
                                ...
                                value={
                                  billingAsMailing
                                    ? mailing_state.input.value
                                    : billingDefaults.billing_state || undefined
                                }
                                persistErrors={persistFieldsErrors}
                                label={`State ${canEdit ? "*" : ""}`}
                              />
                              <Input
                                ...
                                disabled={isBusy || billingAsMailing}
                                value={
                                  billingAsMailing ? mailing_zip.input.value : billingDefaults.billing_zip || undefined
                                }
                                persistErrors={persistFieldsErrors}
                                label={`Zip code ${canEdit ? "*" : ""}`}
                              />
                              <Input
                                ...
                                value={
                                  billingAsMailing
                                    ? mailing_country.input.value
                                    : billingDefaults.billing_country || undefined
                                }
                                persistErrors={persistFieldsErrors}
                                label="Country"
                              />
            ...
            (tab === ContentTabs.Accounts && canManage && (
              <div className={`${styles.section} ${styles.accounts}`}>
                {showTemplate && isTablet && <div class={styles.container_over}>{renderTemplateSection()}</div>}
                {showUserActions && (
              ...
              `}
                    >
                      {(usersTab === UsersTabs.Info && selectedUserId !== null && (
                        <UserManage
                          id={selectedUserId}
                        ...
                        (usersTab === UsersTabs.Permissions && selectedUserId !== null && (
                          <ResourceActions clientId={id} userId={selectedUserId} />
                        ))}
                      {selectedUserId === null && <div className={styles.noUser}>No selected user</div>}
                    </div>
                  </div>
                )) ||
                  (accountsTab === AccountsTabs.Domains && (
