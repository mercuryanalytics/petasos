...
const UserActions = props => {
  ...
  const ensureSelectedItem = useCallback(
    id => {
      ...
      if (id === true || id !== selectedItem || !selectedItem) {
        ...
        const u = (
          toSelect
            ...
            : clientLimit
            ? users.filter(u => u.client_ids.indexOf(clientLimit) > -1)
            : users
        )[0]
  ...
  const setItemStatus = useCallback(
    (parentId, itemId, status) => {
      dispatch(authorizeUser(itemId, parentId, authorizedOptions, { authorize: status }, context)).then(
        () => {},
        () => {}
      )
  ...
  const { form, handleSubmit, submitting } = useForm({
    ...
    onSubmit: values => {
      ...
      dispatch(createUser(result, contextId, mode === UserActionsModes.Manage)).then(
        action => {
  ...
  return (
    <div className={`${styles.container} ${props.className || ""}`}>
      ...
      <Modal className={styles.modal} title="Invite new user" open={isAddUserOpen} onClose={() => handleAddUserClose()}>
        <div className={styles.modalText}>Enter an email address for which to send an invitation.</div>
        <form className={styles.form} onSubmit={handleSubmit}>
          <Input className={`${styles.modalInput} ${styles.stacked}`} field={addUserNameField} label="Name" />
          <Input className={styles.modalInput} field={addUserEmailField} label="Email" />
            ...
            <div className={styles.group}>
              <div className={styles.items}>
                {users
                  .filter(
                    u =>
                      (!limitClientId || u.client_ids.indexOf(limitClientId) > -1) && blockedUsers.indexOf(u.id) === -1
                  )
                  .map(user => (
                    <label
                  ...
                  >
                    ...
                    >
                      ...
                      {!!users.filter(u => u.client_ids.indexOf(client.id) > -1).length && (
                        <div className={styles.groupSettings} onClick={e => handleSettingsToggle(client.id, e)}>
                    ...
                    {!!openClients[client.id] &&
                      (!!loadedClients[client.id] ? (
                        <div className={styles.items}>
                          {users
                            .filter(u => u.client_ids.indexOf(client.id) > -1 && blockedUsers.indexOf(u.id) === -1)
                            .map(user => (
                              <label
                                key={`grant-user-${user.id}`}
                                className={`
