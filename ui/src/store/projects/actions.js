...
export function getProjects(clientId, userId) {
  const queryString = fixMultiQueryString(
    (hasValue(clientId) ? `?client_id=${clientId}` : "") + (hasValue(userId) ? `&user_id=${userId}` : "")
  )
  return dispatch =>
    (!apiCall.isCalled(
      [`${Constants.API_URL}/projects${queryString}`].concat(!hasValue(userId) ? [`${Constants.API_URL}/projects`] : [])
    )
...
export function getOrphanProjects() {
  return dispatch =>
    (!apiCall.isCalled([`${Constants.API_URL}/projects/orphans`])
...
export function getProject(id, clientId, force) {
  return dispatch =>
    (force ||
    !apiCall.isCalled(
      [`${Constants.API_URL}/projects`, `${Constants.API_URL}/projects/${id}`].concat(
        hasValue(clientId) ? [`${Constants.API_URL}/reports?client_id=${clientId}`] : []
      )
    )
...
export function createProject(data) {
  ...
  return dispatch => {
    return apiCall("POST", `${Constants.API_URL}/projects`, { body }).then(
...
export function updateProject(id, data) {
  const body = JSON.stringify({ project: data })
  return dispatch => {
    return apiCall("PATCH", `${Constants.API_URL}/projects/${id}`, { body }).then(
...
export function deleteProject(id) {
  return dispatch => {
    return apiCall("DELETE", `${Constants.API_URL}/projects/${id}`).then(
