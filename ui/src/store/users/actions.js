...
export function getUsers(clientId, force) {
  ...
  return dispatch =>
    (force ||
    !apiCall.isCalled(!queryString ? [`${Constants.API_URL}/users`] : [`${Constants.API_URL}/users${queryString}`])
      ? apiCall("GET", `${Constants.API_URL}/users${queryString}`)
      : queryState(state => ({
          target: state.usersReducer.users,
          filters: [
            {
              run: hasValue(clientId),
...
export function getUser(id) {
  return dispatch =>
    (!apiCall.isCalled([`${Constants.API_URL}/users`, `${Constants.API_URL}/users/${id}`])
...
export function createUser(data, clientId, noAuth) {
  ...
  return dispatch => {
    return apiCall("POST", `${Constants.API_URL}/users${queryString}`, { body }).then(
      res => {
...
export function copyPermissions(id, data) {
  ...
  return dispatch => {
    return apiCall("POST", `${Constants.API_URL}/users/${id}/copy`, { body }).then(
      res => {
...
export function updateUser(id, data) {
  ...
  return dispatch => {
    return apiCall("PATCH", `${Constants.API_URL}/users/${id}`, { body }).then(
...
export function deleteUser(id, clientId) {
  const queryString = hasValue(clientId) ? `?client_id=${clientId}` : ""
  return dispatch => {
    return apiCall("DELETE", `${Constants.API_URL}/users/${id}${queryString}`).then(
...
export function getResearchers() {
  return dispatch =>
    (!apiCall.isCalled([`${Constants.API_URL}/users/researchers`])
...
export function getScopes() {
  return dispatch =>
    (!apiCall.isCalled([`${Constants.API_URL}/scopes`])
...
export function getUserAuthorizations(id) {
  return dispatch =>
    apiCall("GET", `${Constants.API_URL}/users/${id}/authorized`).then(
...
export function getMyAuthorizations(id) {
  return dispatch =>
    (!apiCall.isCalled([`${Constants.API_URL}/users/me`])
...
export function getAuthorizedUsers(contextId, res) {
  ...
  return dispatch =>
    apiCall("GET", `${Constants.API_URL}/${resPath}/${resId}/authorized${queryString}`).then(
...
export function getAllAuthorizedUsers(res) {
  ...
  return dispatch =>
    apiCall("GET", `${Constants.API_URL}/${resPath}/${resId}/authorized`).then(
...
export function authorizeUser(id, contextId, res, states, context = null) {
  ...
  if (res) {
    if (res.isGlobal) {
      return dispatch => {
        return apiCall("POST", `${Constants.API_URL}/users/${id}/scopes`, { body: JSON.stringify(states) }).then(
          async res => {
  ...
  return dispatch => {
    let url =
      context === "project" || context === "report"
    ...
    return apiCall("POST", url, { body: JSON.stringify(data) }).then(
      async res => {
        await refresh(dispatch)
