...
export function getReports(projectId, clientId) {
  const queryString = hasValue(projectId)
    ? `?project_id=${projectId}`
    : hasValue(clientId)
    ? `?client_id=${clientId}`
    : ""
  return dispatch =>
    (!apiCall.isCalled([`${Constants.API_URL}/reports`, `${Constants.API_URL}/reports${queryString}`])
...
export function getOrphanReports() {
  return dispatch =>
    (!apiCall.isCalled([`${Constants.API_URL}/reports/orphans`])
...
export function getClientReports(clientId) {
  return dispatch =>
    (!apiCall.isCalled([`${Constants.API_URL}/clients/${clientId}/orphans`])
...
export function getClientSubReports(clientId) {
  return dispatch =>
    (!apiCall.isCalled([`${Constants.API_URL}/reports?client_id=${clientId}`])
...
export function getReport(id, projectId) {
  return dispatch =>
    (!apiCall.isCalled(
      [`${Constants.API_URL}/reports`, `${Constants.API_URL}/reports/${id}`].concat(
        hasValue(projectId) ? [`${Constants.API_URL}/reports?project_id=${projectId}`] : []
      )
    )
...
export function createReport(data, projectId) {
  ...
  return dispatch => {
    return apiCall("POST", `${Constants.API_URL}/reports`, { body }).then(
      res => {
...
export function updateReport(id, data, projectId) {
  const body = JSON.stringify({ report: data })
  return dispatch => {
    return apiCall("PATCH", `${Constants.API_URL}/reports/${id}`, { body }).then(
      res => {
        dispatch(getProject(projectId, null, true))
...
export function deleteReport(id) {
  return dispatch => {
    return apiCall("DELETE", `${Constants.API_URL}/reports/${id}`).then(
