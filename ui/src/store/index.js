      ...
      if (scopeId && isGlobal) {
        ...
        for (let k = 0; k < resAuthorizations.length; k++) {
          ...
          if (role) {
            ...
            if (canInheritAccess) {
              const accessRole =
                resType === ResourceTypes.Client
                  ? UserRoles.ClientAccess
                  : resType === ResourceTypes.Project
...
export const hasRoleOnClient = (userId, clientId, role) => {
  return isSuperUser(userId) || isUserAuthorized(getAuthorizations(), userId, ResourceTypes.Client, clientId, role)
}

export const hasRoleOnProject = (userId, projectId, role) => {
  return isSuperUser(userId) || isUserAuthorized(getAuthorizations(), userId, ResourceTypes.Project, projectId, role)
}

export const hasRoleOnReport = (userId, reportId, role) => {
  return isSuperUser(userId) || isUserAuthorized(getAuthorizations(), userId, ResourceTypes.Report, reportId, role)
}
